...
tags:
  ...
  - 可视化
cover:
  image: images/blog/ggplot2_relative_funcs.png
...
## 准备
在这里，我需要导入本节需要的包。 tidyverse 包括八个包，其中之一是 ggplot。 primer.data包 拥有比R 内置的更多的数据集。
...
注意了，现在图层即将发生变化。我们选择设置x周、y轴、color的字段。
...
更改geom层的颜色，所以该层紧贴geom层，且在geom层之上。设置方法使用 ``scale_color_manual()``  即可。scale_color_munual()中的values可以传入颜色十六进制的字符串，除了传入颜色字符串。
